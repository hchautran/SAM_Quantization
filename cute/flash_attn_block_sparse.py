"""
Flash Attention v2 forward pass — block-sparse, no RoPE, no rel-pos bias.

Targeted at SAM-2 / Hiera attention, which uses plain SDPA without per-position
bias inside attention. This is a stripped fork of `flash_attn_rope_fused.py`:

  REMOVED (compared with the RoPE-fused variant)
    • m_cos / m_sin GMEM tensors
    • sCosQ / sSinQ / sCosK / sSinK shared-memory buffers
    • the `_rotate_smem_inplace` and `_load_cos_sin_K` helpers
    • the `head_dim % 4 == 0` (axial-RoPE) constraint

  KEPT
    • m_block_mask  — banded-diag + keep-bar block-sparse skip
    • m_perm_Q / m_perm_K — host-managed permutation indices, accepted in
      the API for parity with the SAM-1 sparsesam kernel and so callers can
      pass the perm without conditional plumbing. Q/K/V are expected to be
      pre-permuted on the host (gather), and the kernel does NOT re-index by
      `perm_*` internally — the perm tensors are otherwise unused. The
      output O is in the permuted order; the host scatters it back.

Tensors:
  Q/K/V/O      — (B, S, N, D)
  perm_Q       — (B, Sq) int32  (host-permuted Q layout, indices into original)
  perm_K       — (B, Sk) int32  (same for K — usually equal to perm_Q for self-attn)
  block_mask   — (B, N, ceil(Sq/m_blk), ceil(Sk/n_blk)) int32  (1 = run, 0 = skip)
"""
...
class FlashAttentionForwardAmpereBlockSparse:
    """FA2 with block-sparse masking. No rel-pos bias, no RoPE."""
    ...
    def can_implement(dtype, head_dim, m_block_size, n_block_size, num_threads) -> bool:
        ...
        # SMEM: Q + 2·KV (Q overlaps with O on epilogue).
        smem_q  = m_block_size * head_dim_padded * 2
        smem_kv = n_block_size * head_dim_padded * 2 * 2
        if smem_q + smem_kv > utils.get_smem_capacity_in_bytes("sm_80"):
            return False
    ...
    def __call__(
        ...
        mV: cute.Tensor,
        mO: cute.Tensor,
        m_perm_Q: cute.Tensor,        # (B, Sq) int32 — accepted, unused (host-managed)
        m_perm_K: cute.Tensor,        # (B, Sk) int32 — accepted, unused (host-managed)
        m_block_mask: cute.Tensor,    # (B, N, num_m, num_n) int32
    ...
    def kernel(
        ...
        # Predicates for head-dim padding
        ...
        # K load for the first (right-most) n_block, only if it's enabled.
        ...
        self.compute_one_n_block(
            mQ, mK, m_block_mask, sK, thr_mma, tiled_mma,
            tSrQ, tSrK, tOrVt, acc_O,
            gmem_tiled_copy_QKV, tKVcKV, tKgK, tKsK, tVgV, tVsV, tKVpKV,
            smem_tiled_copy_Q, smem_tiled_copy_K, smem_tiled_copy_V,
            tSsQ, tSrQ_copy_view, tSsK, tSrK_copy_view, tOsVt, tOrVt_copy_view,
    ...
    @cute.jit
    def compute_one_n_block(self,
        mQ, mK, m_block_mask, sK,
        thr_mma, tiled_mma, tSrQ, tSrK, tOrVt, acc_O,
        gmem_tiled_copy_QKV, tKVcKV, tKgK, tKsK, tVgV, tVsV, tKVpKV,
        smem_tiled_copy_Q, smem_tiled_copy_K, smem_tiled_copy_V,
        tSsQ, tSrQ_copy_view, tSsK, tSrK_copy_view, tOsVt, tOrVt_copy_view,
        row_max, row_sum, softmax_scale_log2,
        ...
        # Prefetch K for the next (lower) n-block, only if enabled.
        if n_block_idx > 0:
        ...
        if block_enabled:
            self.softmax_rescale_O(
                mQ, mK, acc_O, thr_mma, row_max, row_sum, softmax_scale_log2,
