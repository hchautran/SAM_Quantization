...
__global__ void flash_attn_rel_kernel(
    const scalar_t*__restrict__ Q,  // B, H, T, D  
    const scalar_t*__restrict__ K,  // B, H, T, D    
    const scalar_t*__restrict__ V,  // B, H, T, D    
    scalar_t *__restrict__ O,
    const scalar_t*__restrict__ rel_pos_h, // B , h, H * W, H 
    const scalar_t*__restrict__ rel_pos_w, // B , h, H * W, W 
    int rel_pos_size,
    int batch_size,
    int num_heads,
    int seq_len,
    ...
    float* rel_pos_h_s = o_s + HEAD_DIM;
    float* rel_pos_w_s = rel_pos_h_s + rel_pos_size;
    ...
    int rel_pos_batch_head_offset = (batch_idx * num_heads + head_idx) * seq_len * rel_pos_size;
    
    // read current q into the share mem and init o to zeros;
    ...
    // read current rel pos into the share mem
    for (int p = tid ; p < rel_pos_size; p+=blockDim.x) {
        rel_pos_h_s[p] = static_cast<float>(rel_pos_h[rel_pos_batch_head_offset + q_idx * rel_pos_size + p]);
        rel_pos_w_s[p] = static_cast<float>(rel_pos_w[rel_pos_batch_head_offset + q_idx * rel_pos_size + p]);
    }
    ...
    for (int tile = 0; tile < CEIL_DIV(seq_len, TILE_SIZE); tile++) {
        ...
        for (int kv_idx = tid; kv_idx < TILE_SIZE; kv_idx+=blockDim.x) {
            int kv_offset = tile * TILE_SIZE + kv_idx;
            int rel_pos_w_idx = kv_offset % rel_pos_size; 
            int rel_pos_h_idx = kv_offset / rel_pos_size;
            if (kv_offset < seq_len) {
                float qk = 0.0f;
                for (int d = 0; d < head_dim; d++) {
                    qk += q_s[d] * K_s[kv_idx*head_dim + d];
                }
                qK_s[kv_idx] = qk * softmax_scale + rel_pos_w_s[rel_pos_w_idx] + rel_pos_h_s[rel_pos_h_idx];
            }
            else {
                qK_s[kv_idx] = -INFINITY;
            }
...
torch::Tensor flash_attn_rel(
    ...
    TORCH_CHECK(rel_pos_h.dim() == 4, "rel pos h  must be 3D");
    TORCH_CHECK(rel_pos_w.dim() == 4, "rel pos w  must be 3D");

    const int batch_size = Q.size(0);
    const int num_heads = Q.size(1);
    const int seq_len = Q.size(2);
    const int head_dim = Q.size(3);
    const int rel_pos_size = rel_pos_w.size(3);
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(Q.scalar_type(), "flash_attn", ([&] {
        flash_attn_rel_kernel<scalar_t, TILE_SIZE, HEAD_DIM><<<grid, block, smem_size>>>(
            Q.data_ptr<scalar_t>(),
            K.data_ptr<scalar_t>(),
            V.data_ptr<scalar_t>(),
            O.data_ptr<scalar_t>(),
            rel_pos_h.data_ptr<scalar_t>(),
            rel_pos_w.data_ptr<scalar_t>(),
            rel_pos_size,
            batch_size,
            num_heads,
            seq_len,
