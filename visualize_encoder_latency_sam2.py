"""
Visualization script for SAM2 encoder latency profiling.

This script profiles the SAM2 Hiera encoder and creates comprehensive visualizations
of attention and MLP layer latencies.

Usage:
    python visualize_encoder_latency_sam2.py --model_config sam2_hiera_l.yaml --checkpoint path/to/sam2_hiera_large.pt
    python visualize_encoder_latency_sam2.py --model_config sam2_hiera_l.yaml --checkpoint path/to/sam2_hiera_large.pt --batch_size 4
"""
import argparse
import numpy as np
import torch
...
from processors.encoder.profiler_sam2 import (
    sam2_encoder_latency_monkey_patch,
    analyze_sam2_encoder_breakdown,
    ProfilerStats,
)
...
def profile_and_collect_data(
    ...
    """
    Profile the SAM2 encoder and collect timing data.
    ...
    sam2_encoder_latency_monkey_patch(predictor.model, profiler, sync_cuda=(device == "cuda"))
    ...
    # Analyze
    results = analyze_sam2_encoder_breakdown(profiler, print_details=False)
...
def plot_per_block_breakdown(profiler: ProfilerStats, output_dir: Path, batch_size: int = 1):
    ...
    # Identify stage transition blocks (blocks with Q pooling have different characteristics)
    stage_blocks = []
    median_attn = np.median(attention_data)
    for i, attn_time in enumerate(attention_data):
        if attn_time > median_attn * 1.5:  # Heuristic for stage transition
            stage_blocks.append(block_indices[i])
    ...
    # Add annotation for stage blocks
    if stage_blocks:
        ax.text(0.02, 0.98, f'Orange = Stage Transition Blocks ({stage_blocks})',
                transform=ax.transAxes, fontsize=10, verticalalignment='top',
                bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))

    plt.tight_layout()
    plt.savefig(output_dir / 'per_block_breakdown_sam2.png', dpi=300, bbox_inches='tight')
    print(f"Saved: {output_dir / 'per_block_breakdown_sam2.png'}")
    plt.close()
...
def plot_overall_breakdown(profiler: ProfilerStats, results: dict, output_dir: Path):
    ...
    title1 = 'SAM2 Encoder Component Breakdown'
    if results.get('batch_size', 1) > 1:
        title1 += f'\n(Batch Size: {results["batch_size"]})'
    ax1.set_title(title1, fontsize=14, fontweight='bold')

    # Pie chart 2: Hiera blocks only
    ...
    ax2.set_title('Hiera Blocks Breakdown\n(Attention vs MLP)', fontsize=14, fontweight='bold')

    plt.tight_layout()
    plt.savefig(output_dir / 'overall_breakdown_sam2.png', dpi=300, bbox_inches='tight')
    print(f"Saved: {output_dir / 'overall_breakdown_sam2.png'}")
    plt.close()
...
def plot_attention_vs_mlp_comparison(profiler: ProfilerStats, output_dir: Path, batch_size: int = 1):
    ...
    ax.set_xlabel('Block Index', fontsize=12, fontweight='bold')
    ax.set_ylabel('Latency (ms)', fontsize=12, fontweight='bold')
    title = 'SAM2 Attention vs MLP Latency per Block'
    if batch_size > 1:
        title += f' (Batch Size: {batch_size})'
    ax.set_title(title, fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'attention_vs_mlp_sam2.png', dpi=300, bbox_inches='tight')
    print(f"Saved: {output_dir / 'attention_vs_mlp_sam2.png'}")
    plt.close()
...
def plot_latency_distribution(profiler: ProfilerStats, output_dir: Path):
    ...
    # Separate attention blocks by type
    attention_times = []
    mlp_times = []
    ...
    data = [attention_times, mlp_times]
    ...
    ax.set_ylabel('Latency (ms)', fontsize=12, fontweight='bold')
    ax.set_title('SAM2 Latency Distribution by Component Type', fontsize=14, fontweight='bold')
    ax.grid(axis='y', alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'latency_distribution_sam2.png', dpi=300, bbox_inches='tight')
    print(f"Saved: {output_dir / 'latency_distribution_sam2.png'}")
    plt.close()
...
def plot_cumulative_time(profiler: ProfilerStats, output_dir: Path):
    ...
    ax.set_title('SAM2 Cumulative Latency Through Hiera Blocks', fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'cumulative_time_sam2.png', dpi=300, bbox_inches='tight')
    print(f"Saved: {output_dir / 'cumulative_time_sam2.png'}")
    plt.close()
...
def plot_heatmap(profiler: ProfilerStats, output_dir: Path):
    ...
    ax.set_xticklabels([f'{i}' for i in block_indices])
    ax.set_xlabel('Block Index', fontsize=12, fontweight='bold')
    ax.set_title('SAM2 Latency Heatmap: Attention and MLP Across Blocks',
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'latency_heatmap_sam2.png', dpi=300, bbox_inches='tight')
    print(f"Saved: {output_dir / 'latency_heatmap_sam2.png'}")
    plt.close()
...
def create_all_visualizations(
    ...
    checkpoint: str = None,
    image_path: str = None,
    output_dir: str = "./profiling_plots_sam2",
    batch_size: int = 1,
    num_runs: int = 10,
    device: str = "cuda",
):
    """
    Run profiling and create all visualizations for SAM2.
    ...
    print("="*80)
    print(f"PROFILING AND VISUALIZING SAM2 ENCODER - {model_config}")
    if batch_size > 1:
    ...
    # Print summary
    analyze_sam2_encoder_breakdown(profiler, print_details=True)
...
def main():
    parser = argparse.ArgumentParser(
        description="Visualize SAM2 encoder latency profiling results"
    )
    parser.add_argument(
        "--model_config",
        type=str,
        default="sam2_configs/sam2/sam2_hiera_l.yaml",
